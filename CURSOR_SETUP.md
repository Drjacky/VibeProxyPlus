...
- VibeProxyPlus running (menu bar, server on port **8317**)
...
## Step 1: Enable Cursor in VibeProxyPlus

1. Open **VibeProxyPlus** settings from the menu bar.
...
3. VibeProxyPlus reads `~/Library/Application Support/Cursor/User/globalStorage/state.vscdb` and writes `~/.cli-proxy-api/cursor.json`.

You can sign in to Cursor IDE once and re-fetch when tokens rotate. VibeProxyPlus also watches that database and re-imports when it changes.
...
## Step 3: Point your client at VibeProxyPlus
...
| Want official VibeProxy releases   | See [automazeio/vibeproxy](https://github.com/automazeio/vibeproxy/releases) (no Cursor) |
